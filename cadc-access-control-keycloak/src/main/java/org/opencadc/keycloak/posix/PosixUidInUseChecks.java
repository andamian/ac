...
/**
 * Collision checks for allocated POSIX UIDs against Keycloak and LDAP stores.
 */
public final class PosixUidInUseChecks {

    private PosixUidInUseChecks() {
    }

    public static boolean isInUseInKeycloakDb(KeycloakSession session, RealmModel realm, int uid) {
        Stream<UserModel> users = session.users().searchForUserByUserAttributeStream(realm,
                PosixAttributeNames.UID_NUMBER, String.valueOf(uid));
        try (users) {
            return users.findAny().isPresent();
        }
    }

    public static boolean isInUseInLdap(LDAPStorageProvider ldapProvider, RealmModel realm, int uid) {
        try (LDAPQuery query = LDAPUtils.createQueryForUserSearch(ldapProvider, realm)) {
            Condition condition = new LDAPQueryConditionsBuilder()
                    .equal(PosixAttributeNames.LDAP_UID_NUMBER, String.valueOf(uid));
