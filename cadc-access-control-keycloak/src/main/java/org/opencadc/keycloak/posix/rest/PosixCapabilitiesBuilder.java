...
/**
 * Builds the VOSI capabilities document for the Keycloak UMS POSIX endpoints.
 */
public final class PosixCapabilitiesBuilder {

    private PosixCapabilitiesBuilder() {
    }

    public static String build(PosixUmsConfig config) {
        return build(config.getPublicBaseUrl(), config.getUidAccessUrl());
    }

    public static String build(String publicBaseUrl, String uidAccessUrl) {
        String base = normalizeBaseUrl(publicBaseUrl);
        String uidUrl = normalizeBaseUrl(uidAccessUrl);
        return ""
                ...
                + "      <accessURL use=\"base\">" + uidUrl + "</accessURL>\n"
                + "      <securityMethod />\n"
